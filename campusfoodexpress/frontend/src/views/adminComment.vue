<template>
    <div class="comment-management" :back="'/admin'">
        ...
        <div class="comment-management-box">
            <!-- 顶部的搜索框 -->
            <a-input placeholder="输入用户ID进行搜索" v-model="searchId" @input="searchComments" style="margin-bottom: 20px" />
            ...
            <a-table :columns="columns" :data="filteredComments" :pagination="false">
                <template #images="{ record }">
                    <div class="image-container">
                        <img v-for="(image, index) in record.images" :key="index" :src="image" alt="评论图片"
                            class="image-preview" @error="onImageError" />
                    </div>
                </template>
