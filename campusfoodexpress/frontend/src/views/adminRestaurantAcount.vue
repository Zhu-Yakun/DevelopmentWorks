  ...
  <div class="restaurant-account">
    ...
    <div class="restaurant-account-box">
      ...
      <a-space style="margin-bottom: 10px">
        <a-input placeholder="输入商家名称进行搜索" v-model="searchId" @input="searchRestaurant" />
    ...
    <!-- 添加/编辑商家模态框 -->
    <a-modal v-model:visible="restaurantModalVisible" :title="modalTitle" @cancel="closeRestaurantModal"
      @before-ok="handleConfirm">
      <a-form ref="restaurantForm" :model="restaurantFormData" :rules="rules">
        <a-form-item field="image" label="商家图片">
          <input type="file" @change="onImageFileChange" style="margin-top: 10px" />
          <img v-if="previewImage.url" :src="previewImage.url" alt="商家图片预览" class="image-preview" />
          <img v-else-if="
            restaurantFormData.image &&
            typeof restaurantFormData.image === 'string'
          " :src="restaurantFormData.image" alt="当前商家图片" class="image-preview" />
        </a-form-item>
        <a-form-item field="name" label="商家名称">
          <a-input v-model="restaurantFormData.name" />
        </a-form-item>


        <!-- 地址栏 -->
        <a-form-item field="address" label="地址">
          <a-input v-model="restaurantFormData.address" @click="showMap = true" />
        </a-form-item>

        <!-- 地图弹窗 -->
        <a-modal v-model:visible="showMap" title="选择商家位置" :width="800" @cancel="showMap = false" :footer="null">
          <!-- 地图组件 -->
        ...
        <a-form-item field="phone" label="电话">
          <a-input v-model="restaurantFormData.phone" />
        </a-form-item>
        <a-form-item field="description" label="介绍">
          <a-input v-model="restaurantFormData.description" />
        </a-form-item>
        <a-form-item field="qr_code" label="二维码信息">
          <input type="file" @change="onQrCodeFileChange" style="margin-top: 10px" />
          <img v-if="previewQrCode.url" :src="previewQrCode.url" alt="二维码预览" class="image-preview" />
          <img v-else-if="
            restaurantFormData.qr_code &&
            typeof restaurantFormData.qr_code === 'string'
          " :src="restaurantFormData.qr_code" alt="当前二维码" class="image-preview" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
